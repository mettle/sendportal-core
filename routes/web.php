...
Route::middleware(['auth', 'verified'])->group(static function (Router $router) {
    // Auth.
    Route::namespace('Auth')->group(static function (Router $router) {
        // Logout.
        $router->get('logout', ['LoginController@logout'])->name('logout');

        // Profile.
        Route::prefix('profile')->name('profile.')->group(static function (Router $router) {
    ...
    // Settings.
    $router->prefix('settings')->group(static function (Router $settingsRouter) {
